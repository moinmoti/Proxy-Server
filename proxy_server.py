...
class Server:
	...
	print("Listening at port ", config['BIND_PORT'])
        while True:
	...
	filename = temp[webserver_pos:]
	    ...
	    line1 = requestSplitted[0].split()
	    line1[1] = filename
	    line1 = ' '.join(line1)
	    requestSplitted[0]=line1
	    requestNew = '\n'.join(requestSplitted)
            print('REQUEST ',requestNew)
	    s.send(requestNew)                           # send request to webserver
